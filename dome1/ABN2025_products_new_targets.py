...
# ==================== DEBUG TRACKING START ====================
TARGET_MISSING_PROJECTS = [
    "保利商业保理有限公司2025年度第五期邦鑫一号资产支持票据",
    "深圳前海联易融商业保理有限公司2025年度第一期中悦一号资产支持票据",
    "广州市自来水有限公司2025年度2号第一期绿色资产支持票据(科创票据)"
]

PROJECT_TRACKER = {name: [] for name in TARGET_MISSING_PROJECTS}

def is_target_match(name):
    """Check if name matches any target project using fuzzy logic"""
    try:
        norm_name = name.replace(" ", "").replace("(", "").replace(")", "").replace("（", "").replace("）", "")
        for target in TARGET_MISSING_PROJECTS:
            norm_target = target.replace(" ", "").replace("(", "").replace(")", "").replace("（", "").replace("）", "")
            if norm_target in norm_name or norm_name in norm_target:
                return True, target
    except Exception as e:
        print(f"Error in is_target_match: {e}")
    return False, None

def log_project_event(name, event):
    """Log event for target projects using fuzzy matching"""
    try:
        matched, target = is_target_match(name)
        if matched:
            timestamp = datetime.now().strftime('%H:%M:%S')
            PROJECT_TRACKER[target].append(f"{timestamp} - {event}")
            print(f"🔍 [TRACKING] Matched '{target}': {event}")
    except Exception as e:
        print(f"Error in log_project_event: {e}")
# ==================== DEBUG TRACKING END ====================
    ...
    for pageIndex in range(1, MAX_PAGES + 1):
        ...
        for i in pageItems:
            ...
            if should_process:
                ...
                # TRACKING IN LIST
                log_project_event(title, f"Found in API list (Page {pageIndex}). Date: {release_date.strftime('%Y-%m-%d')}")
...
def newProd(url):
    ...
    is_target, target_name = is_target_match(fullName)
    
    if is_target:
        log_project_event(fullName, f"Parsed fullName: {fullName} (MATCHED TARGET)")
    else:
        # If user wants ONLY target projects, we return here
        # print(f"Skipping non-target project: {fullName}")
        return

    rawShortName = df.iloc[1][3]
    try:
        # 尝试解析标准ABN简称格式 (例如: 24保利ABN001)
        if "ABN" in rawShortName and len(rawShortName) > 5:
            ...
            # 格式不匹配，使用原始值
            print(f"  ⚠️ 非标准简称格式: {rawShortName}，使用原始值")
            shortName = rawShortName
    except Exception as e:
        print(f"  ⚠️ 解析简称出错 '{rawShortName}': {e}，使用原始值")
        shortName = rawShortName
    offer_type = df.iloc[-1][3]
    
    # DEBUG PRINT requested by user
    print(f"DEBUG: Last Row Columns (2 and 3 index): {df.iloc[-1][2]}, {df.iloc[-1][3]}")
    ...
    if res:
        ...
        log_project_event(TrustName, "Trust table record exists. Proceeding to check documents...")
        ...
        try:
            ...
            log_project_event(TrustName, "Inserted into Trust table")
        except Exception as e:
            print("err1", e)
            log_project_event(TrustName, f"Error inserting into Trust table: {e}")
            # print(TrustName)
            # print(insertTrust)
            return
    ...
    if result is None:
        ...
        log_project_event(TrustName, "Error: Could not retrieve TrustId after insertion")
    ...
    print(f"✓ 开始尝试处理文档 (CollectionMethod={CollectionMethod})...")
    ...
    if q1:
        ...
        if not smsExist:
            ...
            log_project_event(TrustName, "Warning: No '说明书' found in docs")
...
def insert_into_db(product_name, q, typy_q):
    ...
    if "募集说明书" in q[1]:
        log_project_event(TrustCode, f"Processing Offering Circular: {q[1]}")
        ...
        try:
             cursor.execute(sql_i1)
             log_project_event(TrustCode, "Inserted into DisclosureOfInformation")
        except Exception as e:
             # print(f"DisclosureOfInformation insert skipped/failed: {e}")
             if "2627" in str(e): # Primary key violation usually
                 log_project_event(TrustCode, "DisclosureOfInformation record already exists")
             else:
                 log_project_event(TrustCode, f"DisclosureOfInformation insert error: {e}")
        ...
        # 检查ProductsStateInformation是否已存在
        ...
        if cursor.fetchone():
             ...
             log_project_event(TrustCode, f"Task already exists in ProductsStateInformation (DocID={tdid})")
        else:
            # 插入ProductsStateInformation
            log_project_event(TrustCode, f"Creating new task in ProductsStateInformation (DocID={tdid})...")
            ...
            try:
                ...
                log_project_event(TrustCode, "Task creation successful")
            except Exception as e:
                print(f"❌ ProductsStateInformation插入失败: {e}")
                print(f"SQL: {sql_i2}")
                log_project_event(TrustCode, f"Task creation failed: {e}")
                # 不抛出异常，继续处理
...
if __name__ == "__main__":
    ...
    # ==================== DEBUG SUMMARY START ====================
    print(f"\n{'='*80}")
    print("📋 遗漏产品最终状态报告 (MISSING PROJECTS REPORT)")
    print(f"{'='*80}")
    
    found_count = 0
    not_found_count = 0
    
    for name, events in PROJECT_TRACKER.items():
        print(f"\n项目: {name}")
        if events:
            found_count += 1
            print("  状态: ✅ 脚本已触达")
            ...
            not_found_count += 1
            print("  状态: ❌ 脚本未在列表或处理中遇到此产品")
            print("  可能原因: API未返回此数据，或标题/项目名不匹配")

    print(f"\n{'='*80}")
    print(f"追踪总结: 触达 {found_count} 个, 未触达 {not_found_count} 个")
