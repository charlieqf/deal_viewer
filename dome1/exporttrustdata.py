...
def get_trust_name(tid):
    try:
        sql = "select TrustName from TrustManagement.Trust where TrustId = {0}".format(tid)
        name = dbh.exec_fetch_one(sql, conn_name="PortfolioManagement")
        return name if name else "Unknown"
    except Exception as e:
        print("Warning: Failed to get TrustName: " + str(e))
        return "Unknown"

get_trust_name(trustId)
...
# 文件压缩用于下载
fileCount=0
trust_name = get_trust_name(trustId)
# Sanitize filename (remove invalid chars)
safe_trust_name = re.sub(r'[\\/*?:"<>|]', '_', str(trust_name)).strip()
zip_file_name = "{0}_{1}.zip".format(trustId, safe_trust_name)
