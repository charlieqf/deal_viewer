...
def list_ftp_directory_with_retry(ftp, path, retries=6):
    for attempt in range(retries):
        ...
        except (ftplib.error_temp, ftplib.error_perm, ftplib.error_reply, BrokenPipeError, TimeoutError, ConnectionError, socket.timeout) as e:
            print(
                f"Error occurred while listing directory: {e}. Retrying {retries - attempt - 1} more times."
            )
            # 如果是超时错误，可能需要重新连接FTP
            if isinstance(e, (TimeoutError, socket.timeout, ConnectionError)) and attempt < retries - 1:
                try:
                    print("Connection timed out, attempting to reconnect FTP...")
                    # 保存原始连接信息
                    host = ftp.host
                    port = ftp.port
                    user = ftp._user if hasattr(ftp, '_user') else FTP_USER  # 备用用户名
                    passwd = ftp._passwd if hasattr(ftp, '_passwd') else FTP_PASS  # 备用密码
