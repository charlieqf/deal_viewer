...
def update_pdf():
    ...
    # with open(r'C:\DataTeam\Products_log\更新时间TXT记录\pdf_time.txt', 'r') as f:
    #     last_date = f.read()
    #     print("上次更新的日期为" + last_date)
    # with open(r'C:\DataTeam\Products_log\更新时间TXT记录\pdf_time.txt', 'w') as f:
    #     f.write(first_update_times)
    for update_time, update_link in zip(update_times, update_links):
        if update_link.get('href') not in ('/Info/161117925','/Info/161136562','/Info/161136635'):
            continue
        data = {
        ...
        if 1:
            ...
            with open(r'C:\DataTeam\Products_log\更新时间TXT记录\fx_FileName.txt', 'a') as f:
                f.write(now_time)
                f.write(Product_name)
                f.write('\n')
                f.close()
            with open(r'C:\DataTeam\Products_log\更新时间TXT记录\发行文件.xlsx', 'a') as f:
                f.write(now_time)
                f.write(Product_name)
                f.write('\n')
                f.close()
            ...
            # 发行产品写入
            with open(r'C:\DataTeam\Products_log\更新时间TXT记录\中国债券网产品.xlsx', 'a') as f:
                f.write(Product_name)
                f.write('\n')
                f.close()
                logger.info("Finish")
...
def now_products(Product_name):
    ...
    try:
        ...
        TrustCode1 = STrustCode.split('2022')[0]
        TrustCode2 = STrustCode.split('2022')[-1]

        TrustCode = TrustCode1 + '_' + FCode + '2022' + TrustCode2
        print(TrustCode)
        if '更正' in Product_name or '更新' in Product_name:
            return
    ...
    # if Product_name=='华驭第十二期汽车抵押贷款支持证券发行文件':
    #     TrustName='华驭第十二期汽车抵押贷款支持证券'
    #     TrustNameShort='华驭-12'
    #     TrustCode='HuaYu_AUTO-12'
...
if __name__ == '__main__':
    ...
    if os.path.exists(r'C:\DataTeam\Products_log\更新时间TXT记录\发行文件.xlsx') == True:
        os.remove(r'C:\DataTeam\Products_log\更新时间TXT记录\发行文件.xlsx')
    gzProd, cmProd = [], []
    use_selenium()
    update_pdf()
    ...
    with open(r'C:\DataTeam\Products_log\更新时间TXT记录\err_fxwj.pkl', 'wb') as f:
